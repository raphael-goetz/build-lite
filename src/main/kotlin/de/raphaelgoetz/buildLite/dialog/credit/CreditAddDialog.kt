...
private fun Player.yesAction(recordWorld: RecordWorld): ActionButton {
    return ActionButton.create(
        Component.text("Confirm"), Component.text("Add this player as a credited builder"), 100, DialogAction.customClick(
            { view, _ ->
...
private fun Player.noAction(recordWorld: RecordWorld): ActionButton {
    return ActionButton.create(
        Component.text("Discard"), Component.text("Go back without saving changes."), 100, DialogAction.customClick(
...
fun Player.createCreditAddDialog(recordWorld: RecordWorld): Dialog {
    val nameInput = DialogInput.text(FIELD_NAME_KEY, Component.text("Enter Player Name")).build()

    val base = DialogBase.builder(Component.text("Add Credit")).inputs(listOf(nameInput)).build()
